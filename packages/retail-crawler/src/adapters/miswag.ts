...
const MOCK_CATALOG: Array<{
...
}> = [
  {
    ...
    priceIqd: 479_000,
    url: "https://www.miswag.co/samsung-galaxy-a54-5g-128gb",
  },
  {
    keywords: ["iphone", "13"],
    title: "Apple iPhone 13 128GB",
    priceIqd: 885_000,
    url: "https://www.miswag.co/apple-iphone-13-128gb",
  },
  {
    keywords: ["lenovo", "ideapad"],
    title: "Lenovo IdeaPad Slim 3 15",
    priceIqd: 619_000,
    url: "https://www.miswag.co/lenovo-ideapad-slim-3-15",
  },
  {
    keywords: ["lg", "refrigerator", "fridge"],
    title: "LG GN-B472PQMB Refrigerator",
    priceIqd: 1_140_000,
    url: "https://www.miswag.co/lg-gn-b472pqmb",
  },
];
...
export class MiswagAdapter implements RetailAdapter {
  ...
  private async searchLive(title: string): Promise<RetailObservation[]> {
    await throttle("miswag", 2000);

    // Call Miswag's catalog search endpoint (mocked or structured)
    const searchUrl = `https://api.miswag.co/v1/search?q=${encodeURIComponent(title)}`;
    ...
    if (!response.ok) {
      throw new Error(`Miswag search returned status ${response.status}`);
    }
    ...
    if (!payload.products || payload.products.length === 0) {
      throw new Error("Miswag search returned no matching products");
    }
